...
void starttyping(char name[])
{
    ...
    if(choice1==1)
    {
	    update_progresscard(name,level,score,speed);
...
void update_progresscard(char name[],int level,int score,int speed)
{
    FILE *fp;
	fp=fopen("file.txt","a");
	...
	fprintf(fp,"\nname=%s   level=%d    score=%d    speed=%d",name,level,score,speed);
	fclose(fp);
